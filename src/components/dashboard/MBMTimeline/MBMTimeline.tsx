...
import { Phone, Users, TrendingUp, TrendingDown, Minus, Star, AlertCircle, Eye, Zap, Filter, BookOpen, Calendar, Building2, User, CalendarDays, Banknote, Target, CheckCircle2, XCircle } from 'lucide-react';
import { Text, Badge, Card, Modal, Drawer } from '@/components/common/atoms';
import { Customer, ContentEngagement, SalesAction } from '@/types/customer';
...
// 주간 타임라인 정보 (월요일 기준)
const WEEKS = [
  { key: '1104', label: '11월 1주', range: '11/4~10', startDate: '2024-11-04', endDate: '2024-11-10', isCurrent: false },
  { key: '1111', label: '11월 2주', range: '11/11~17', startDate: '2024-11-11', endDate: '2024-11-17', isCurrent: false },
  { key: '1118', label: '11월 3주', range: '11/18~24', startDate: '2024-11-18', endDate: '2024-11-24', isCurrent: false },
  { key: '1125', label: '11월 4주', range: '11/25~12/1', startDate: '2024-11-25', endDate: '2024-12-01', isCurrent: false },
  { key: '1202', label: '12월 1주', range: '12/2~8', startDate: '2024-12-02', endDate: '2024-12-08', isCurrent: false },
  { key: '1209', label: '12월 2주', range: '12/9~15', startDate: '2024-12-09', endDate: '2024-12-15', isCurrent: true },
  { key: '1216', label: '12월 3주', range: '12/16~22', startDate: '2024-12-16', endDate: '2024-12-22', isCurrent: false },
  { key: '1223', label: '12월 4주', range: '12/23~29', startDate: '2024-12-23', endDate: '2024-12-29', isCurrent: false },
] as const;
...
// 신뢰레벨에 따른 색상
const getTrustLevelVariant = (level: string | null): 'success' | 'warning' | 'error' | 'default' => {
  switch (level) {
...
export const MBMTimeline = ({ data, timePeriod: _timePeriod }: MBMTimelineProps) => {
  ...
  // 모달 열기
  const openContentModal = (customer: Customer, week: typeof WEEKS[number], trustChange: number) => {
    setModalData({
      customer,
  ...
  // 액션 모달 열기
  const openActionModal = (customer: Customer, action: SalesAction, weekLabel: string) => {
    setActionModalData({ customer, action, weekLabel });
  ...
  // 신뢰지수 변화 계산
  const getTrustChange = (customer: Customer, weekKey: WeekKey, index: number) => {
  ...
  const getLastAttendedMBMWeekIndex = (customer: Customer): number => {
    ...
    WEEKS.forEach((week, index) => {
      ...
      if (mbmEvent && customer.attendance?.[mbmEvent.key as keyof typeof customer.attendance]) {
        lastIndex = index;
      }
    });
  ...
  const filteredData = useMemo(() => {
    ...
    return baseData.filter(customer => getNudgeType(customer) === activeFilter);
    ...
    <div className={styles.container}>
      ...
      <div className={styles.filterBar}>
        <div className={styles.filterLabel}>
          <Filter size={14} />
          <Text variant="label" color="tertiary">필터</Text>
        </div>
        <div className={styles.filterButtons}>
          {FILTER_OPTIONS.map((option) => {
            ...
            return (
              <button
                key={option.value}
                className={`${styles.filterButton} ${isActive ? styles.active : ''} ${option.color ? styles[`filter_${option.color}`] : ''}`}
      ...
      <div className={styles.header}>
        <div className={styles.companyHeader}>
          <Text variant="label" color="tertiary">기업명</Text>
        </div>
        <div className={styles.timelineHeader}>
          {WEEKS.map((week) => {
            ...
            return (
              <div 
                key={week.key} 
                className={`${styles.weekColumn} ${hasMBM ? styles.mbmWeek : ''} ${week.isCurrent ? styles.currentWeek : ''}`}
              >
                <Text variant="label" weight="semibold" color={hasMBM ? 'accent' : week.isCurrent ? 'success' : 'secondary'}>
                  {week.label}
                </Text>
                ...
                {hasMBM && mbmEvent && (
                  <div className={styles.mbmBadge}>
                    <Star size={10} />
                    <Text variant="caption" weight="semibold" color="accent">{mbmEvent.label}</Text>
                  </div>
                )}
                {week.isCurrent && (
      ...
      <div className={styles.timeline}>
        {filteredData.map((customer) => {
          ...
          return (
            <Card key={customer.no} className={`${styles.customerRow} ${nudgeType ? styles[`nudge_${nudgeType}`] : ''}`} padding="none">
              {/* 기업 정보 */}
              <div className={styles.companyInfo}>
                <div className={styles.companyMain}>
                  <div className={styles.companyNameRow}>
                    <button 
                      ...
                      title="클릭하여 상세 정보 보기"
                    >
                      <Text variant="body-sm" weight="semibold">{customer.companyName}</Text>
                    </button>
                    {nudgeType === 'contact' && (
                      <div className={styles.contactBadge} title={`MBM 이후 신뢰지수 +${trustChangeAfterMBM} 상승! 지금 컨택하세요`}>
                        <Zap size={12} />
                        <Text variant="caption" weight="semibold">컨택 필요</Text>
                      </div>
                    )}
                    {nudgeType === 'interest' && (
                      <div className={styles.interestBadge} title={`MBM 이후 신뢰지수 +${trustChangeAfterMBM} 상승`}>
                        <Eye size={12} />
                        <Text variant="caption" weight="semibold">관심 필요</Text>
                      </div>
                    )}
                    {nudgeType === 'action' && (
                      <div className={styles.actionBadge} title="MBM 참석 이후 팔로업 액션이 없습니다">
                        <AlertCircle size={12} />
                        <Text variant="caption" weight="semibold">액션 권장</Text>
                      </div>
                    )}
                  </div>
                ...
                <div className={styles.currentTrust}>
                  <Text variant="caption" color="tertiary">현재</Text>
              ...
              <div className={styles.timelineCells}>
                {WEEKS.map((week, index) => {
                  ...
                  const mbmAttended = mbmEvent && customer.attendance?.[mbmEvent.key as keyof typeof customer.attendance];
                  // 현재 주에만 넛지 표시
                  ...
                  return (
                    <div 
                      key={week.key} 
                      className={`${styles.cell} ${hasMBM ? styles.mbmCell : ''} ${week.isCurrent ? styles.currentCell : ''} ${showNudge ? styles[`nudgeCell_${nudgeType}`] : ''}`}
                    >
                      {/* MBM 참석 표시 */}
                        ...
                        <div className={styles.trustInfo}>
                          <div className={styles.trustIndex}>
                            ...
                            {trustChange !== null && trustChange !== 0 && (() => {
                              const weekContents = trustChange > 0 ? getContentsForPeriod(customer, week.startDate, week.endDate) : [];
                              const hasContents = weekContents.length > 0;
                              return (
                                <button
                                  className={`${styles.change} ${trustChange > 0 ? styles.up : styles.down} ${hasContents ? styles.clickable : ''}`}
                                  onClick={() => hasContents && openContentModal(customer, week, trustChange)}
                      ...
                      {actions.length > 0 && (
                        <div className={styles.actions}>
                          {actions.map((action, i) => (
                            <button 
                              key={i} 
                              className={`${styles.action} ${styles[action.type]}`}
                              onClick={() => openActionModal(customer, action, week.label)}
                              title="클릭하여 상세 정보 확인"
                            >
                              {action.type === 'call' ? (
                                <Phone size={10} />
                              ...
                              <Text variant="caption" className={styles.actionText}>
                      ...
                      {actions.length === 0 && !mbmAttended && showNudge && (
                        <div className={`${styles.cellNudge} ${styles[`cellNudge_${nudgeType}`]}`}>
                          {nudgeType === 'contact' && <><Zap size={12} /><Text variant="caption" weight="medium">지금 컨택!</Text></>}
                          {nudgeType === 'interest' && <><Eye size={12} /><Text variant="caption" weight="medium">관심 필요</Text></>}
                          {nudgeType === 'action' && <><AlertCircle size={12} /><Text variant="caption" weight="medium">액션 권장</Text></>}
                        </div>
                      )}
                      {actions.length === 0 && !mbmAttended && !showNudge && !trustData && (
      ...
      <div className={styles.legend}>
        <div className={styles.legendItem}>
          ...
          <Text variant="caption" color="tertiary">MBM 참석</Text>
        </div>
        <div className={styles.legendItem}>
          <Phone size={12} className={styles.legendCall} />
          <Text variant="caption" color="tertiary">콜</Text>
        </div>
        <div className={styles.legendItem}>
          <Users size={12} className={styles.legendMeeting} />
          <Text variant="caption" color="tertiary">미팅</Text>
        </div>
        <div className={styles.legendItem}>
          <Zap size={12} className={styles.legendContact} />
          <Text variant="caption" color="tertiary">컨택 필요 (+10↑)</Text>
        </div>
        <div className={styles.legendItem}>
          <Eye size={12} className={styles.legendInterest} />
          <Text variant="caption" color="tertiary">관심 필요 (상승)</Text>
        </div>
        <div className={styles.legendItem}>
          <AlertCircle size={12} className={styles.legendAction} />
          <Text variant="caption" color="tertiary">액션 권장</Text>
        </div>
      </div>
      ...
      >
        {modalData && (() => {
          const filteredContents = getContentsForPeriod(modalData.customer, modalData.startDate, modalData.endDate);
          return (
            <div className={styles.contentModal}>
              <div className={styles.contentHeader}>
                <div className={styles.periodBadge}>
                  <Text variant="body-sm" weight="semibold">{modalData.weekLabel}</Text>
                  <Text variant="caption" color="tertiary">({modalData.weekRange})</Text>
                </div>
                <div className={styles.contentStats}>
                  ...
                  <div className={styles.statItem}>
                    ...
                    <Text variant="h4" weight="bold" mono color="success">+{modalData.trustChange}</Text>
                  </div>
                </div>
      ...
      <Modal
        ...
        title={actionModalData ? `${actionModalData.customer.companyName}` : '영업 액션'}
        size="sm"
      >
        {actionModalData && (
          <div className={styles.actionModal}>
            ...
            <div className={styles.actionModalContent}>
              ...
              <Text variant="body" weight="medium">{actionModalData.action.content}</Text>
            </div>
            ...
            <div className={styles.actionModalMeta}>
              ...
              <div className={styles.metaItem}>
                ...
                <Text variant="body-sm">{actionModalData.customer.manager}</Text>
              </div>
              <div className={styles.metaItem}>
                <Text variant="caption" color="tertiary">신뢰레벨</Text>
                <Badge variant={getTrustLevelVariant(actionModalData.customer.trustLevel)} size="sm">
                  {actionModalData.customer.trustLevel}
                </Badge>
              </div>
            </div>
          </div>
        ...
        {selectedCompany && (
          <div className={styles.companyDrawer}>
            ...
            <section className={styles.drawerSection}>
              <Text variant="label" color="tertiary" className={styles.sectionTitle}>기본 정보</Text>
              <div className={styles.infoGrid}>
                <div className={styles.infoItem}>
                  <Building2 size={14} />
                  <Text variant="caption" color="tertiary">기업 규모</Text>
                  <Badge variant="default" size="sm">{selectedCompany.companySize || '-'}</Badge>
                </div>
                <div className={styles.infoItem}>
                  <Target size={14} />
                ...
                <div className={styles.infoItem}>
                  ...
                  <Text variant="caption" color="tertiary">재계약일</Text>
                  <Text variant="body-sm">{selectedCompany.renewalDate || '-'}</Text>
                </div>
                <div className={styles.infoItem}>
                  <Banknote size={14} />
                  <Text variant="caption" color="tertiary">계약금액</Text>
                  <Text variant="body-sm" weight="semibold">{formatCurrency(selectedCompany.contractAmount)}</Text>
                </div>
                <div className={styles.infoItem}>
                  <Star size={14} />
                  <Text variant="caption" color="tertiary">사용 제품</Text>
            ...
            {/* 신뢰 지표 */}
            <section className={styles.drawerSection}>
              <Text variant="label" color="tertiary" className={styles.sectionTitle}>신뢰 지표</Text>
              <div className={styles.trustStats}>
                <div className={styles.trustStatCard}>
                  <Text variant="caption" color="tertiary">신뢰지수</Text>
                  <Text variant="h3" weight="bold" mono>{selectedCompany.trustIndex ?? '-'}</Text>
                </div>
                <div className={styles.trustStatCard}>
                  <Text variant="caption" color="tertiary">신뢰레벨</Text>
                  <Badge variant={getTrustLevelVariant(selectedCompany.trustLevel)} size="md">
                    {selectedCompany.trustLevel || '-'}
                  </Badge>
                </div>
                <div className={styles.trustStatCard}>
                  <Text variant="caption" color="tertiary">변화량</Text>
                  <div className={styles.changeIndicator}>
                    {selectedCompany.changeDirection === 'up' ? (
                      <><TrendingUp size={16} className={styles.upIcon} /><Text variant="h4" weight="bold" color="success">+{selectedCompany.changeAmount}</Text></>
                    ) : selectedCompany.changeDirection === 'down' ? (
                      <><TrendingDown size={16} className={styles.downIcon} /><Text variant="h4" weight="bold" color="error">-{selectedCompany.changeAmount}</Text></>
                    ) : (
                      <Text variant="h4" weight="bold" color="tertiary">-</Text>
                    )}
                  </div>
                </div>
              </div>
            ...
            {/* 영업 파이프라인 */}
            <section className={styles.drawerSection}>
              <Text variant="label" color="tertiary" className={styles.sectionTitle}>영업 파이프라인</Text>
              <div className={styles.pipelineCards}>
                {/* 신뢰형성 */}
                <div className={styles.pipelineCard}>
                  <div className={styles.pipelineHeader}>
                    <Text variant="body-sm" weight="semibold">신뢰형성</Text>
                    <Badge 
                      variant={selectedCompany.trustFormation.customerResponse === '상' ? 'success' : selectedCompany.trustFormation.customerResponse === '중' ? 'warning' : 'error'} 
                      size="sm"
                    >
                      {selectedCompany.trustFormation.customerResponse}
                    </Badge>
                  </div>
                  <Text variant="caption" color="secondary">{selectedCompany.trustFormation.detail}</Text>
                  {selectedCompany.trustFormation.interestFunction && (
                    <div className={styles.interestTag}>
                      <Text variant="caption" color="accent">관심: {selectedCompany.trustFormation.interestFunction}</Text>
                    </div>
                  )}
                </div>
                ...
                <div className={styles.pipelineCard}>
                  <div className={styles.pipelineHeader}>
                    <Text variant="body-sm" weight="semibold">가치인식</Text>
                    <Badge variant="info" size="sm">{selectedCompany.valueRecognition.possibility}</Badge>
                  </div>
                  <div className={styles.checkList}>
                    <div className={`${styles.checkItem} ${selectedCompany.valueRecognition.test ? styles.checked : ''}`}>
                      {selectedCompany.valueRecognition.test ? <CheckCircle2 size={12} /> : <XCircle size={12} />}
                      <Text variant="caption">테스트</Text>
                    </div>
                    <div className={`${styles.checkItem} ${selectedCompany.valueRecognition.quote ? styles.checked : ''}`}>
                      {selectedCompany.valueRecognition.quote ? <CheckCircle2 size={12} /> : <XCircle size={12} />}
                      <Text variant="caption">견적</Text>
                    </div>
                    <div className={`${styles.checkItem} ${selectedCompany.valueRecognition.approval ? styles.checked : ''}`}>
                      {selectedCompany.valueRecognition.approval ? <CheckCircle2 size={12} /> : <XCircle size={12} />}
                      <Text variant="caption">품의</Text>
                    </div>
                    <div className={`${styles.checkItem} ${selectedCompany.valueRecognition.contract ? styles.checked : ''}`}>
                      {selectedCompany.valueRecognition.contract ? <CheckCircle2 size={12} /> : <XCircle size={12} />}
                ...
                <div className={styles.pipelineCard}>
                  <div className={styles.pipelineHeader}>
                    <Text variant="body-sm" weight="semibold">도입결정</Text>
                    <Badge variant="info" size="sm">{selectedCompany.adoptionDecision.possibility}</Badge>
                  </div>
                  {selectedCompany.adoptionDecision.targetRevenue && (
                    <Text variant="body-sm" weight="semibold" color="success">
                      목표: {formatCurrency(selectedCompany.adoptionDecision.targetRevenue)}
                    </Text>
                  )}
                  {selectedCompany.adoptionDecision.targetDate && (
                    <Text variant="caption" color="tertiary">예상 시기: {selectedCompany.adoptionDecision.targetDate}</Text>
                  )}
                </div>
              </div>
            </section>
            ...
            {/* 영업 액션 히스토리 */}
            <section className={styles.drawerSection}>
              <Text variant="label" color="tertiary" className={styles.sectionTitle}>영업 액션 ({selectedCompany.salesActions?.length || 0}건)</Text>
              <div className={styles.actionHistory}>
                {selectedCompany.salesActions?.map((action, idx) => (
                  <div key={idx} className={styles.actionHistoryItem}>
                    <div className={`${styles.actionIcon} ${styles[action.type]}`}>
                      {action.type === 'call' ? <Phone size={12} /> : <Users size={12} />}
                    </div>
                ...
                {(!selectedCompany.salesActions || selectedCompany.salesActions.length === 0) && (
                  <Text variant="body-sm" color="tertiary">등록된 액션이 없습니다.</Text>
                )}
              </div>
            </section>

            {/* 컨텐츠 조회 기록 */}
            {selectedCompany.contentEngagements && selectedCompany.contentEngagements.length > 0 && (
              <section className={styles.drawerSection}>
                <Text variant="label" color="tertiary" className={styles.sectionTitle}>컨텐츠 조회 ({selectedCompany.contentEngagements.length}건)</Text>
