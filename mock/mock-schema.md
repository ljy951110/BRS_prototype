...
**신뢰지수는 콘텐츠 소비(TOFU/MOFU/BOFU)와 MBM 참석에 따라 증가/감소합니다.**
...
| **워크샵**        | +3              | 실습 중심 오프라인 행사 |
| **컨퍼런스**      | +4              | 대규모 컨퍼런스 참석    |
| **1:1 미팅**      | +3              | 담당자와 1:1 미팅       |
| **제품 데모**     | +4              | 제품 데모 세션 참석     |
| **네트워킹 행사** | +2              | 네트워킹 이벤트 참석    |

> **중요**: 신뢰지수 변동을 분석할 때는 `contentEngagements` (콘텐츠 조회 기록)와 `mbmAttendances` (MBM 참석 기록)를 함께 확인해야 합니다. 영업 액션(`salesActions`)은 신뢰지수가 아닌 가능성(`possibility`)과 고객반응(`customerResponse`)에 영향을 미칩니다.
...
| `targetDate`       | string \| null | 액션 후 변경된 목표일자 | `"12월"`, `"1월"`, `"12월말"`  |
...
> **참고**: MBM 이벤트는 특정 날짜에 진행되며, 신뢰지수는 주간 단위로 별도 업데이트됩니다. MBM 참석 시 신뢰지수가 +3 증가합니다.
...
```
┌─────────────────────────────────────────────────────────────────┐
│                   고객 활동 (Customer Activity)                 │
├─────────────────────────────────────────────────────────────────┤
│  콘텐츠 소비 (contentEngagements)                               │
│    TOFU (+1p) / MOFU (+2p) / BOFU (+3p)                        │
│                                                                 │
│  MBM 참석 (mbmAttendances)                                      │
│    웨비나 (+2p) / 워크샵 (+3p) / 컨퍼런스 (+4p)                  │
│    1:1미팅 (+3p) / 제품데모 (+4p) / 네트워킹 (+2p)              │
│                        ↓                                        │
│              ┌────────────────┐                                │
│              │   신뢰지수     │                                │
│              │  (trustIndex)  │                                │
...
| **신뢰지수 변동** | 콘텐츠 소비 + MBM 참석 히스토리  | `contentEngagements`, `mbmAttendances` |
